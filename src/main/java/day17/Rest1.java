    ...
    public ModelAndView getUserDetails(@RequestParam String email,String password) {
        ...
        if (userProfile.containsKey(email)) {
            if (userProfile.get(email).getPassword().equals(password)) {
                ...
                if(tweets.isEmpty())
                    fetchTweets();
                List<List<Tweet>> tweet = new ArrayList<>();
                List<Tweet>list1=new ArrayList<>();
                for (Map.Entry entry : tweets.entrySet()) {
                    tweet.add((List<Tweet>) entry.getValue());
                    for(int i=0;i< tweet.size();i++)
                    {
                        list1.add((Tweet) tweet.get(i));
                    }
                }
                // ArrayList<Tweet> t= new ArrayList<Tweet>(tweets.values());

                modelAndView.getModel().put("users", list1);
                System.out.println(list1+" users data  ");
                return modelAndView;
            }
            else {
